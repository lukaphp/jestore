package it.computerproject.jestore.web.rest;
...
import it.computerproject.jestore.config.KafkaProperties;
...
class JeStoreKafkaResourceIT {
    ...
    void setup() {
        ...
        JeStoreKafkaResource kafkaResource = new JeStoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/je-store-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/je-store-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
